...
def simulate_stress_strain(E, sigma_y, K, n, L_0, A_0, strain_max=0.3, num_points=100):
    '''
    This function simulates both engineering stress-strain and true stress-strain curves 
    using Hooke's Law + Hollomon's Law (expanded version).

    Parameters:
        E (float): Elastic modulus in MPa
        sigma_y (float): Yield stress (strength) in MPa
        ...
        strain_max (float): Maximum strain to simulate. Default value is set to 0.3 (30%).
        num_points (int): Number of data points to simulate. Default value is set to 100.
    ...
    yield_strain = sigma_y / E  # Calculate yield strain (e_y) from yield strength (σ_y ) and elastic modulus (E)
    
    # Apply linear elastic region until yield point, then plastic region using the expanded version of Hollomon's law
    ...
    # The code calculates values for engineering stress above by applying the np.where function (in a compact, vectorized way) conditionally:
